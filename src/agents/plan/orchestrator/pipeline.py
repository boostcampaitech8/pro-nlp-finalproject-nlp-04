"""
오케스트레이터 파이프라인 - 전체 기획서 생성 워크플로우 조합
"""
...
from agents.plan.orchestrator.logger import get_logger, reset_logger, LogLevel
...
def run_plan(structured_input_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    print("=" * 60)
    print(f"Blueprint 기반 기획서 생성 파이프라인 시작")
    print(f"스타일: {structured_input_data.get('planning_style')}")
    print("=" * 60)
    
    # 재귀 제한 증가
    result = pipeline.invoke(initial_state, {"recursion_limit": 200})
